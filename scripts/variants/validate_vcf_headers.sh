#!/bin/bash

######################################################################
# Script: validate_vcf_headers.sh
# Description: Verifies if all VCF files in a given directory
#              have the same sample order in the #CHROM line.
#
# Usage:
#     ./validate_vcf_headers.sh -d /path/to/vcf_directory
#
# Parameters:
#     -d    Path to the directory containing .vcf files
#
# Author: BAHS Team (Cenicaña)
######################################################################

show_help() {
    echo "Usage: $0 -d <vcf_directory>"
    exit 1
}

while getopts "d:" opt; do
    case $opt in
        d) vcf_dir="$OPTARG" ;;
        *) show_help ;;
    esac
done

if [ -z "$vcf_dir" ]; then
    show_help
fi

if [ ! -d "$vcf_dir" ]; then
    echo "Error: Directory '$vcf_dir' does not exist."
    exit 1
fi

echo "Starting header comparison in directory: $vcf_dir"

reference_header=""

for file in "$vcf_dir"/*.vcf; do
    # Extract the #CHROM line and columns from sample 10 onwards
    header=$(grep "^#CHROM" "$file" | cut -f10-)

    if [ -z "$reference_header" ]; then
        reference_header="$header"
        echo "Reference header set from: $(basename "$file")"
    else
        if [ "$header" != "$reference_header" ]; then
            echo "WARNING: Sample order mismatch found in file: $(basename "$file")"
        else
            echo "OK: $(basename "$file")"
        fi
    fi
done

echo "Comparison completed."
